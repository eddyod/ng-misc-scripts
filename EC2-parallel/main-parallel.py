"""
Script to run parallel processing of CZI Files
"""
...
for i in range(0, min(NUM_INSTANCES, len(s3_file_list))):
...
if EMPTY_S3:
    os.system(f"aws s3 del --recursive s3://{S3_BUCKET}/{S3_CZI_DIR}")
    os.system(f"aws s3 del --recursive {S3_OUTPUT_DIR}")
